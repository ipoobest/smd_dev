...
export const actions = {
  async createStudent({ commit }, object) {
    return new Promise(async (resolve, reject) => {
      try {
        const result = this.$axios.$post(`${process.env.parseUrl}/classes/${classes.students}`, object)
  ...
  async getStudent({ commit }) {
    return new Promise(async (resolve, reject) => {
      try {
        const result = this.$axios.$get(`${process.env.parseUrl}/classes/${classes.students}`)
  ...
  async getStudentsByClass({ commit }, classId) {
    return await this.getStudents({ class: classId })
  },
